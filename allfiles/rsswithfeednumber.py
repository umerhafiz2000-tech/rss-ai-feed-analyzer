...
DATABASE_NAME = 'feedswithnumbercyber_security_feeds'
COLLECTION_NAME = 'feedswithnumberfeed_entries'
...
class RSSAggregator:
    ...
    def update_all_feeds(self) -> Dict[str, int]:
        ...
        # Loop through feeds with counter
        for idx, feed_url in enumerate(feed_urls, start=1):
        ...
        logger.info(f"Feed update completed in {elapsed_time:.2f} seconds. "
                    f"Explored {stats['total_feeds']} feeds. "
                    f"Stats: {stats}")
